...
#	Version: 1.0
#	Github: https://github.com/owogo/Ehcoo
...
landing_config(){
	...
	if [ ! -f $full_sysctl_dir ]; then
		wget https://cdn.jsdelivr.net/gh/owogo/Ehcoo/ehco-landing.service -O ehco.service
...
forward_config(){
	...
	if [ ! -f "/root/ehco.json" ]; then
		wget https://cdn.jsdelivr.net/gh/owogo/Ehcoo/ehco.json -O ehco.json
	fi
	JSON='{"listen":"0.0.0.0:local_port","listen_type":"raw","transport_type":"wss","tcp_remotes":["wss://ip:landing_port"],"udp_remotes":["ip:landing_port"]}'
	...
	echo $temp > ehco.json
	if [ ! -f $full_sysctl_dir ]; then
		wget https://cdn.jsdelivr.net/gh/owogo/Ehcoo/ehco-forward.service -O ehco.service
		mv ehco.service $sysctl_dir
	fi
	echo "正在本机启动Echo隧道"
	systemctl daemon-reload
...
更新内容及反馈： https://github.com/owogo/Ehcoo
————————————模式选择————————————
